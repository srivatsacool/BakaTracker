...
import { Flame, ListTodo, Target, BookOpen, Compass, Cloud, CloudOff, Settings as SettingsIcon, X, Sun, Moon, ChevronLeft, ChevronRight, Download, WifiOff } from 'lucide-react';
...
export const Layout: React.FC = () => {
  ...
  const { stats, settings, syncStatus, syncError, habits, habitLogs, tasks, journal, syncWithSheets, setSheetsUrl, setApiKey, theme, toggleTheme, setAccentColors } = useStore();
    ...
    <div className="min-h-screen bg-bg-primary text-text-primary flex flex-col md:flex-row">
      ...
      <aside className={`hidden md:flex flex-col ${isCollapsed ? 'w-20' : 'w-64'} bg-surface border-r-4 border-black p-4 transition-all duration-300 shrink-0 justify-between relative`}>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className={`neo-card ${isCollapsed ? 'p-2 items-center' : 'p-4'} bg-accent-pink/10 flex flex-col gap-3 transition-all`}>
            {isCollapsed ? (
