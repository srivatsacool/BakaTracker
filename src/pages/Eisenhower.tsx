    ...
    <div className="relative cabinet !overflow-visible !shadow-none p-3 flex flex-col gap-2 transition hover:scale-[1.01]" style={{ background: 'rgba(242,242,242,0.04)', border: '1px solid rgba(242,242,242,0.1)', borderRadius: 10 }}>
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover/card:opacity-100 transition-opacity shrink-0 group-hover/card:!opacity-100" style={{ opacity: 0 }}>
          <button onClick={onMoveToKanban} title="Open in Kanban" className="p-1 rounded cursor-pointer hover:bg-white/10" style={{ border: '1px solid var(--obs-glass-15)' }}>
            <ExternalLink className="w-3 h-3" />
          </button>
          <button onClick={onDelete} title="Delete task" className="p-1 rounded cursor-pointer hover:bg-white/10 hover:text-danger" style={{ border: '1px solid var(--obs-glass-15)' }}>
...
export const Eisenhower: React.FC = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto flex flex-col gap-6">
      ...
      {/* Quadrant counts — real numbers from the store */}
      <div className="flex flex-wrap gap-1.5" aria-label="Quest counts per quadrant">
        {QUADRANTS.map(q => (
          <span key={q.key} className="chip font-mono text-[10px]" style={{ color: q.accent, borderColor: `${q.accent}44`, background: `${q.accent}10` }}>
            {q.emoji} {q.label} · {getQuadrantTasks(q.key).length}
          </span>
        ))}
        <span className="chip font-mono text-[10px]" style={{ color: 'var(--arcade-paper-muted)' }}>
