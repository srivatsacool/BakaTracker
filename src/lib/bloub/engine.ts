/*
 * Vendored from Bloub (https://github.com/jeremy-prt/bloub) src/bot/engine.ts
...
/**
 * Ou le bot porte son regard quand quelque chose d'exterieur le pilote â€” le
 ...
 *   yeux suivait celle de chaque expression â€” Â« neutre Â» regarde a +28,6deg quand
 *   les autres sont entre -9 et +9 â€” donc les yeux tombaient d'un coup au premier
 ...
 * confondent pas : quand le pointeur bouge, la derive doit s'eteindre â€” cumulees,
 ...
 * derriere la boule et revenir de l'autre cote â€” et `-360deg` etant le meme
...
export class BotEngine {
   ...
   * son inertie â€” le regard n'atteint jamais tout a fait un curseur qui bouge.
   */
   ...
   * chaque rattrapage â€” le suivi tremblerait au lieu de glisser.
   *
  ...
  setLook(look: Look | null, now: number, morph = BotEngine.LOOK_MORPH) {
     ...
     * reposerait plus jamais. C'est arrive pour de vrai â€” un
   ...
   * â€” c'est la meme cause, donc ce doit etre le meme mouvement.
   *
  ...
  private decalageAtTime(now: number, state: StateId): { x: number; y: number } {
     ...
     * courbe. Jamais sur la valeur interpolee â€” celle-la n'a pas d'identite et n'existe
   ...
   *
   * C'est ce que veut dire Â« rembobiner Â» pour ce moteur. `setState` seul ne peut pas le
   ...
   * et l'export GIF s'ouvrait sur une boule sans yeux â€” la comete a un `eyeAlpha` nul.
   *
   ...
   * a son propre temps ecoule â€” donc encore en train de s'animer, ce qui est voulu.
   */
  private origine(
    now: number,
    shape: number[] | null,
   ...
   * l'etat qu'on quitte pendant tout le fondu â€” le Â« ! Â» d'`alert` se figerait en pleine
   * course â€” alors qu'il n'y a rien a corriger hors morph : l'etat quitte y est deja
  ...
  sample(now: number): BotFrame {
    ...
    // rejouable â€” relire une date d'avant la fin du fondu ne le retrouverait
    ...
    if (origine) {
      ...
      // donnerait un ratio negatif, que l'ease-out extrapole â€” la silhouette
      ...
      // de l'etat quitte, que `setState` renseigne toujours en meme temps que l'origine â€”
    ...
    const gaze = {
      ...
      // melange, sinon la cible l'annulerait en meme temps que la pose â€” or elle
