...
export const Journey: React.FC = () => {
  ...
  const statConfig = [
    { name: 'Discipline', icon: '⚔️', value: stats.discipline, color: 'bg-indigo-500', barColor: '#B07DE8' },
    { name: 'Health', icon: '💪', value: stats.health, color: 'bg-success', barColor: '#6FD08C' },
    { name: 'Knowledge', icon: '🧠', value: stats.knowledge, color: 'bg-blue-500', barColor: '#2BA0FF' },
    { name: 'Creativity', icon: '🎨', value: stats.creativity, color: 'bg-accent-pink', barColor: '#E05252' },
    { name: 'Career', icon: '💼', value: stats.career, color: 'bg-warning', barColor: '#FFD60A' }
  ];
  ...
  const getSourceBadge = (source: string) => {
    switch (source) {
      ...
      case 'task': return 'bg-blue-500/15 border-blue-500 text-blue-500';
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-black font-mono bg-accent-violet/15 text-amber-100 px-2 py-0.5 rounded border border-accent-violet/40 uppercase">
            ...
            return (
              <div key={stat.name} className="p-3 glass-subtle rounded-xl border border-glass-border flex flex-col gap-1.5">
                <div className="flex justify-between items-center border-b border-black/10 dark:border-white/10 pb-0.5">
                  ...
                  <span className="text-[9px] font-mono font-black bg-accent-violet/15 text-amber-200 px-1 py-0.2 rounded border border-accent-violet/40">
                    Lvl {statLvl}
                  </span>
                </div>
                {/* Visual HUD bar */}
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-card p-5 md:col-span-2 flex flex-col gap-4 text-text-primary">
          <div className="border-b border-glass-border pb-2 flex justify-between items-center">
            ...
            <span className="text-xs font-mono font-black bg-accent-violet/15 text-amber-200 px-2 py-0.5 rounded border border-accent-violet/40">
          ...
          <div className="h-48 min-h-48 mt-2 rounded p-1 bg-[linear-gradient(to_right,#e5e5e5_1px,transparent_1px),linear-gradient(to_bottom,#e5e5e5_1px,transparent_1px)] bg-[size:16px_16px] dark:bg-[linear-gradient(to_right,#3d3c38_1px,transparent_1px),linear-gradient(to_bottom,#3d3c38_1px,transparent_1px)]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="sleep" name="Sleep Hours" fill="#6FD08C" stroke="#000000" strokeWidth={2} radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
