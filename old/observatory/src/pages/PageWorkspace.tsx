...
const SaveStatusBadge: React.FC<{ status: SaveStatus }> = ({ status }) => {
  ...
  switch (status.kind) {
    case 'saving':
      return (
        <span className={`${base} chip chip--gold`}>
          <RefreshCw className="w-3.5 h-3.5 animate-spin" aria-hidden="true" /> Saving…
        </span>
      );
    case 'saved':
      return (
        <span className={`${base} chip chip--green`}>
    ...
    case 'error':
      return (
        <span className={`${base} chip chip--red`}>
...
const ConflictBanner: React.FC<{ onReload: () => void; onOverwrite: () => void }> = ({ onReload, onOverwrite }) => (
  <div
    ...
    style={{ background: 'rgba(111, 91, 216,0.08)', borderBottom: '1px solid rgba(111, 91, 216,0.35)' }}
    role="alert"
  >
