...
export const Layout: React.FC = () => {
  ...
  const [inputAccentLight, setInputAccentLight] = useState(settings.accent_color_light || '#FF9F43');
  const [inputAccentDark, setInputAccentDark] = useState(settings.accent_color_dark || '#2BA0FF');
  ...
  const handleResetColors = () => {
    setInputAccentLight('#FF9F43');
    setInputAccentDark('#FF9F43');
  };
  ...
  return (
    <div className="min-h-screen bg-transparent text-text-primary flex flex-col md:flex-row app-canvas">
      ...
      <aside
        ...
        style={{
          background: 'linear-gradient(180deg, #1d1d24 0%, #17171c 40%, #121217 100%)',
          borderColor: 'rgba(232,230,225,0.08)',
          boxShadow: 'inset -1px 0 0 rgba(255,159,67,0.05)',
        }}
      >
        {/* Toggle Sidebar Collapse Button */}
        <button
          onClick={handleToggleCollapse}
          className="absolute top-4 -right-3.5 rounded-full p-1 transition hidden md:block z-10 cursor-pointer border border-amber-400/30"
          style={{
            background: 'rgba(255, 159, 67, 0.15)',
            boxShadow: '0 0 16px rgba(255, 159, 67, 0.25)',
          }}
          title={isCollapsed ? "Expand Shelf" : "Collapse Shelf"}
        >
          {isCollapsed ? <ChevronRight className="w-4 h-4 text-amber-200" /> : <ChevronLeft className="w-4 h-4 text-amber-200" />}
        </button>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div id="sidebar-logo" className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} transition-all`}>
            <div className="relative">
              <img src="/logo.png" alt="BakaTracker Logo" className="w-10 h-10 rounded-xl border border-amber-400/30 shadow-lg object-cover" style={{ boxShadow: '0 0 20px rgba(255,159,67,0.25)' }} />
            </div>
            {!isCollapsed && (
              <div className="transition-all duration-300">
                <h1 className="text-xl font-bold tracking-tight m-0 leading-none text-gradient">BakaTracker</h1>
                <span className="text-[10px] font-mono text-slate-400">Life RPG · Slot 01</span>
              </div>
            )}
          </div>

          {/* Save File Card (character stats) */}
          <div
            id="sidebar-level-bar"
            className={`rounded-2xl border ${isCollapsed ? 'p-2 items-center flex flex-col gap-2.5' : 'p-4 flex flex-col gap-3'} transition-all`}
            style={{
              background: 'linear-gradient(180deg, rgba(255,159,67,0.08) 0%, rgba(43,160,255,0.04) 100%)',
              borderColor: 'rgba(255,159,67,0.22)',
              boxShadow: '0 4px 20px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,220,180,0.08)',
            }}
          >
            {isCollapsed ? (
              <div className="flex flex-col items-center gap-2.5">
                <span
                  className="font-bold font-mono text-xs px-1.5 py-0.5 rounded border text-amber-200"
                  style={{
                    background: 'rgba(255, 159, 67, 0.15)',
                    borderColor: 'rgba(255, 159, 67, 0.4)',
                    boxShadow: '0 0 12px rgba(255, 159, 67, 0.25)',
                  }}
                ...
                <button
                  onClick={() => toggleTheme()}
                  className="p-1.5 rounded border border-white/15 transition hover:border-amber-400/50"
                ...
                <button
                  ...
                  onClick={() => {
                    setInputAccentLight(settings.accent_color_light || '#FF9F43');
                    setInputAccentDark(settings.accent_color_dark || '#2BA0FF');
                    openSettingsModal();
                  }}
                  className="p-1.5 rounded border border-white/15 transition hover:border-amber-400/50"
                  style={{ background: 'rgba(255,255,255,0.05)' }}
                ...
                {isGuest ? (
                  <button
                    ...
                    style={{
                      background: 'linear-gradient(180deg, #FF9F43 0%, #E07B2F 100%)',
                      borderColor: 'rgba(255,220,180,0.35)',
                      boxShadow: '0 0 16px rgba(255, 159, 67, 0.35)',
                    }}
              ...
              // Expanded Save File Card layout
              <>
                <div className="flex justify-between items-center">
                  <span
                    className="font-bold font-mono text-sm px-2 py-0.5 rounded border text-amber-200"
                    style={{
                      background: 'rgba(255, 159, 67, 0.15)',
                      borderColor: 'rgba(255, 159, 67, 0.4)',
                      boxShadow: '0 0 12px rgba(255, 159, 67, 0.25)',
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex justify-between text-xs font-mono font-bold">
                    ...
                    <span className="text-amber-300">{stats.xp} / {settings.xp_per_level}</span>
                  </div>
                  <div
                  ...
                  >
                    <div 
                      ...
                      style={{
                        ...
                        background: 'linear-gradient(90deg, #FF9F43 0%, #FFD60A 100%)',
                        boxShadow: '0 0 12px rgba(255, 159, 67, 0.5)',
                      }}
                    />
                  </div>
                </div>
                ...
                <div id="sidebar-day-progress" className="flex justify-between items-center mt-1 pt-2" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
                  <span className="text-xs font-bold font-mono text-slate-400">Day Clear:</span>
                  <span
          ...
          {isGuest && !isCollapsed && (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-black text-[10px] leading-tight text-amber-200 uppercase">Trial Cart</p>
                <p className="font-mono text-[9px] text-slate-400 leading-tight mt-0.5">
                  Exploring with sample data.
                  {isAuthConfigured ? (
                    <button onClick={() => login()} className="ml-1 underline font-bold hover:text-amber-300 cursor-pointer">
                      Sign in to sync
                    </button>
                  ) : (
                    ' Data stays local.'
          ...
          {/* Navigation — the cartridges on the shelf */}
          <nav className="flex flex-col gap-2">
            {navItems.map(item => {
              ...
              return (
                <Link
                  ...
                  id={itemId}
                  title={item.name}
                  className={`cart-nav-item flex items-center ${isCollapsed ? 'justify-center px-2' : 'gap-3 px-4'} py-3 rounded-xl border font-bold transition-all ${
                    isActive
                      ? 'text-amber-100 border-amber-400/50 cart-nav-item-active'
                      : 'border-transparent text-slate-400 hover:text-white hover:border-white/15'
                  }`}
                  style={{
                    background: isActive
                      ? 'linear-gradient(180deg, rgba(255,159,67,0.22) 0%, rgba(255,159,67,0.08) 100%)'
                      : 'rgba(255,255,255,0.02)',
                    boxShadow: isActive
                      ? '0 0 20px rgba(255, 159, 67, 0.3), inset 0 1px 0 rgba(255,220,180,0.15)'
                      : 'none',
                  }}
                >
            ...
            {/* Desktop PWA Install — a new cart */}
            {deferredPrompt && (
              <button
                onClick={handleInstallPWA}
                className={`flex items-center ${isCollapsed ? 'justify-center px-2' : 'gap-3 px-4'} py-3 rounded-xl border font-bold text-white transition-all cursor-pointer mt-2 border-amber-400/40`}
                style={{
                  background: 'linear-gradient(180deg, rgba(255,159,67,0.2) 0%, rgba(43,160,255,0.1) 100%)',
                  boxShadow: '0 0 20px rgba(255, 159, 67, 0.25), inset 0 1px 0 rgba(255,255,255,0.08)',
                }}
                title="Install BakaTracker Desktop App"
              >
                <Download className="w-5 h-5 shrink-0 text-amber-300" />
      ...
      <div className={`md:hidden flex flex-col w-full ${isEditorRoute ? 'h-screen overflow-hidden' : 'min-h-screen'}`}>
        ...
        <header
          ...
          style={{
            background: 'linear-gradient(180deg, #1d1d24 0%, #17171c 100%)',
            borderColor: 'rgba(255,159,67,0.15)',
          }}
        >
          <div className="flex items-center gap-2">
            <img src="/logo.png" alt="BakaTracker Logo" className="w-8 h-8 rounded-lg border border-amber-400/30 object-cover" style={{ boxShadow: '0 0 14px rgba(255,159,67,0.25)' }} />
          ...
          <div className="flex items-center gap-2">
            <span
              className="font-mono text-xs font-bold px-1.5 py-0.5 rounded border text-amber-200"
              style={{
                background: 'rgba(255, 159, 67, 0.15)',
                borderColor: 'rgba(255, 159, 67, 0.4)',
              }}
            ...
            {deferredPrompt && (
              <button
                onClick={handleInstallPWA}
                className="p-1 rounded border transition cursor-pointer border-amber-400/40"
                style={{
                  background: 'rgba(255, 159, 67, 0.15)',
                }}
            ...
            <button
              onClick={() => toggleTheme()}
              className="p-1 rounded border transition border-white/15 hover:border-amber-400/50"
              style={{ background: 'rgba(255,255,255,0.05)' }}
              title={`Switch to ${theme === 'light' ? 'Dark' : 'Light'} Mode`}
            >
              {theme === 'light' ? <Moon className="w-3.5 h-3.5 text-white" /> : <Sun className="w-3.5 h-3.5 text-white" />}
            ...
            <button
              ...
              className="p-1 rounded border border-white/15 hover:border-amber-400/50"
              style={{ background: 'rgba(255,255,255,0.05)' }}
              title="Settings"
            >
              <SettingsIcon className="w-3.5 h-3.5 text-white" />
        ...
        {!isEditorRoute && (
          <nav
            className="fixed bottom-0 left-0 right-0 py-2 px-2 flex justify-around items-center z-50 border-t"
            style={{
          ...
          {navItems.map(item => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex flex-col items-center justify-center min-h-[48px] min-w-[48px] px-3 py-1.5 rounded-xl transition-all active:scale-95 border ${
                  isActive
        ...
        >
          <div
            ...
            style={{
              ...
              boxShadow: '0 12px 48px rgba(0,0,0,0.6), 0 0 40px rgba(255,159,67,0.08), inset 0 1px 0 rgba(255,220,180,0.08)',
            }}
          >
            <div className="flex justify-between items-center pb-2" style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              <button
                ...
                className="p-1 rounded transition border border-white/10 hover:border-amber-400/40 cursor-pointer"
              ...
              <div className="flex flex-col gap-3 pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
                <div className="flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="text-[10px] font-mono font-bold border border-white/15 px-2 py-0.5 rounded backdrop-blur-xl transition hover:border-amber-400/40 cursor-pointer"
                    style={{ background: 'rgba(255,255,255,0.06)' }}
                  >
                    Reset Defaults
                  </button>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-1.5 ml-auto overflow-x-auto no-scrollbar py-1">
                      {['#FF9F43', '#E05252', '#FFD60A', '#6FD08C', '#2BA0FF', '#B07DE8'].map(c => (
                        <button
                          key={c}
                          type="button"
                ...
                <div className="flex flex-col gap-2 pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
                  ...
                  <button
                    ...
                    className="self-start px-3 py-1.5 rounded-lg border font-bold text-xs transition disabled:opacity-50 backdrop-blur-xl border-white/15 hover:border-amber-400/40"
                    style={{ background: 'rgba(255,255,255,0.06)' }}
                  >
                    ...
                    <>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg border font-bold text-xs transition disabled:opacity-50 backdrop-blur-xl border-white/15 hover:border-amber-400/40"
                          style={{ background: 'rgba(255,255,255,0.06)' }}
                        >
                ...
                <div className="flex justify-end gap-3 mt-2">
                  <button
                    type="button"
                    onClick={() => setShowSettingsModal(false)}
                    className="px-4 py-2 border border-white/15 font-bold rounded-lg backdrop-blur-xl transition text-sm hover:border-amber-400/40"
                    style={{ background: 'rgba(255,255,255,0.06)' }}
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    style={{ background: 'linear-gradient(135deg, #FF9F43 0%, #E07B2F 100%)' }}
                  >
                    Save preferences
                  </button>
                </div>
            ...
            <div className="pt-4 flex flex-col gap-4 mt-2" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
              ...
              <div className="flex flex-col gap-2">
                              <button
                                ...
                                style={{
                                  background: 'linear-gradient(180deg, rgba(255,159,67,0.22) 0%, rgba(43,160,255,0.12) 100%)',
                              ...
                              <button
                                ...
                                className="w-full px-4 py-2 rounded-lg border border-white/15 font-bold text-xs backdrop-blur-xl transition flex items-center justify-center gap-2 hover:border-amber-400/40"
                                style={{ background: 'rgba(255,255,255,0.06)' }}
                              >
