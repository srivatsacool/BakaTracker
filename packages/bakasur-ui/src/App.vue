...
import BotTile from './editor/BotTile.vue'
import Customizer from './editor/Customizer.vue'
import BloubBot from './runtime/BloubBot.vue'
import BakasurLab from './lab/BakasurLab.vue'
import ScenePlayground from './cinematic/Playground.vue'
import ExportBar from './editor/ExportBar.vue'
import CycleDialog from './editor/CycleDialog.vue'
import GifDialog from './editor/GifDialog.vue'
import Settings from './editor/Settings.vue'
import SideRail, { type ViewId } from './editor/SideRail.vue'
import Timeline from './editor/Timeline.vue'
...
import { COLOR_BY_ID, DEFAULT_COLOR, DEFAULT_SHAPE, SHAPE_BY_ID } from './engine/skins'
import { POSES, SEQUENCE, STATES, type StateId } from './engine/states'
...
function readHash() {
  ...
  return {
    ...
    // `#lab` : le studio d'authoring Bakasur. Defaut si pas de hash.
    lab: params.has('lab') || !location.hash || location.hash === '#',
    labPreset: params.get('preset'),
    // `#scene` (+ `&scene=<id>`) : le terrain de jeu cinématique. Le marqueur
    // porte la CLE de l'id : on prend la premiere valeur non vide de `scene`.
    cinema: params.has('scene'),
    cinemaId: [...params.getAll('scene')].find((v) => v) ?? null,
...
const lab = ref(initial.lab)
const cinema = ref(initial.cinema)
const cinemaId = ref(initial.cinemaId)
...
const intro = ref(
  ...
  initial.arrivee ||
    introDue({
      named: initial.named,
      gallery: initial.gallery || initial.lab || initial.cinema,
...
window.addEventListener('hashchange', () => {
  ...
  lab.value = next.lab
  cinema.value = next.cinema
  cinemaId.value = next.cinemaId
  if (next.gallery || next.lab || next.cinema) return
...
<template>
  ...
  <div v-else-if="lab">
    <BakasurLab :initial-preset="initial.labPreset" />
  </div>

  <div v-else-if="cinema">
    <ScenePlayground :initial-scene="cinemaId" />
  </div>
